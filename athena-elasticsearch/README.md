...
* **Domain** - A name Amazon Athena uses to associate with the endpoint of your Elasticsearch
 instance. It is also used internally for the database name. For Elasticsearch instances 
...
3. **domain_mapping** - This is the mapping between the domain names and their associated 
endpoints (in the format: `domain1=endpoint1,domain2=endpont2`). It is only needed when 
...
Elasticsearch instance. The username and password can be stored using the following formats:
...
qualified for nested objects (e.g. `address.street`, where street is a nested field inside an 
address structure).
