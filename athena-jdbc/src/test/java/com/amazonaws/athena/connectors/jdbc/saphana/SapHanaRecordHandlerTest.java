 ...
 * #L%
 */
package com.amazonaws.athena.connectors.jdbc.saphana;
...
public class SapHanaRecordHandlerTest
{
            ...
            throws SQLException
    {
        ...
        schemaBuilder.addField(FieldBuilder.newBuilder("part_id", Types.MinorType.VARCHAR.getType()).build());
        Schema schema = schemaBuilder.build();

        Split split = Mockito.mock(Split.class);
        Mockito.when(split.getProperties()).thenReturn(Collections.singletonMap("part_id", "1"));
        Mockito.when(split.getProperty(Mockito.eq("part_id"))).thenReturn("1");
        ...
        String expectedSql = "SELECT `testCol1`, `testCol2`, `testCol3`, `testCol4`, `testCol5`, `testCol6`, `testCol7`, `testCol8` FROM `TESTSCHEMA`.`TESTTABLE` PARTITION(1)  WHERE (`testCol1` IN (?,?)) AND ((`testCol2` >= ? AND `testCol2` < ?)) AND ((`testCol3` > ? AND `testCol3` <= ?)) AND (`testCol4` = ?) AND (`testCol5` = ?) AND (`testCol6` = ?) AND (`testCol7` = ?) AND (`testCol8` = ?)";
