...
public class MySqlIntegTest extends IntegrationTestBase
{
    ...
    {
        String connectionString = String.format("mysql://jdbc:mysql://%s:%s/%s?user=%s&password=%s",
                endpoint.getAddress(), endpoint.getPort(), mysqlDbName, username, password);
     ...
     * @param endpoint Contains the DB hostname and port information.
     */
    private void createDbSchema(Endpoint endpoint)
    {
        logger.info("----------------------------------------------------");
        ...
        String connectionStringTag = lambdaFunctionName + "_connection_string";
        Map<String, String> tempEnvVars = ImmutableMap.of("default", connectionString,
                connectionStringTag, connectionString);
        JdbcTableUtils jdbcUtils =
                new JdbcTableUtils(lambdaFunctionName, null, tempEnvVars, jdbcProperties);

        jdbcUtils.createDbSchema(mysqlDbName);
    }
    ...
    private void setUpMoviesTable()
    {
        ...
        JdbcTableUtils moviesTable =
                new JdbcTableUtils(lambdaFunctionName, mysqlTableMovies, environmentVars, jdbcProperties);
    ...
    private void setUpBdayTable()
    {
        ...
        JdbcTableUtils bdayTable =
                new JdbcTableUtils(lambdaFunctionName, mysqlTableBday, environmentVars, jdbcProperties);
