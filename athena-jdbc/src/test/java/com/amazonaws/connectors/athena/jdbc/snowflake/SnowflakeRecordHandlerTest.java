...
public class SnowflakeRecordHandlerTest extends TestBase
{
    ...
    public void setup()
    {
        ...
        final DatabaseConnectionConfig databaseConnectionConfig = new DatabaseConnectionConfig("TESTCATALOG", JdbcConnectionFactory.DatabaseEngine.SNOWFLAKE,
                "snowflake://jdbc:snowflake://hostname/user=A&password=B");
            ...
            throws SQLException
    {
        ...
        String expectedSql = "SELECT \"TESTCOL1\", \"TESTCOL2\", \"TESTCOL3\", \"TESTCOL4\", \"TESTCOL5\", \"TESTCOL6\", \"TESTCOL7\", \"TESTCOL8\", \"TESTCOL9\" FROM \"TESTSCHEMA\".\"TESTTABLE\"  WHERE (\"TESTCOL1\" IN (?,?)) AND ((\"TESTCOL2\" >= ? AND \"TESTCOL2\" < ?)) AND ((\"TESTCOL3\" > ? AND \"TESTCOL3\" <= ?)) AND (\"TESTCOL4\" = ?) AND (\"TESTCOL5\" = ?) AND (\"TESTCOL6\" = ?) AND (\"TESTCOL7\" = ?) AND (\"TESTCOL8\" = ?) AND (\"TESTCOL9\" = ?)";
            ...
            throws SQLException
    {
        ...
        String expectedSql = "SELECT \"testDate\" FROM \"TESTSCHEMA\".\"TESTTABLE\"  WHERE (\"testDate\" = ?)";
        PreparedStatement expectedPreparedStatement = Mockito.mock(PreparedStatement.class);
        Mockito.when(this.connection.prepareStatement(Mockito.eq(expectedSql))).thenReturn(expectedPreparedStatement);
