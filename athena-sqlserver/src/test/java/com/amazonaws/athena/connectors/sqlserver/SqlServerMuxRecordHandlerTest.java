...
public class SqlServerMuxRecordHandlerTest
{
    ...
    public void setup()
    {
        this.sqlServerRecordHandler = Mockito.mock(SqlServerRecordHandler.class);
        this.recordHandlerMap = Collections.singletonMap(SqlServerConstants.NAME, this.sqlServerRecordHandler);
        ...
        this.jdbcRecordHandler = new SqlServerMuxRecordHandler(this.amazonS3, this.secretsManager, this.athena, this.jdbcConnectionFactory, databaseConnectionConfig, this.recordHandlerMap, com.google.common.collect.ImmutableMap.of("spill_bucket", "asdf_spill_bucket_loc"));
            ...
            throws Exception
    {
        ...
        ReadRecordsRequest readRecordsRequest = ReadRecordsRequest.newBuilder().setCatalogName(SqlServerConstants.NAME).build();
        this.jdbcRecordHandler.readWithConstraint(blockAllocator, blockSpiller, readRecordsRequest, queryStatusChecker);
        Mockito.verify(this.sqlServerRecordHandler, Mockito.times(1)).readWithConstraint(Mockito.eq(blockAllocator), Mockito.eq(blockSpiller), Mockito.eq(readRecordsRequest), Mockito.eq(queryStatusChecker));
    }
