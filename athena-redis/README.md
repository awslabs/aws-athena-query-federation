...
1. **redis-endpoint** - (required) The hostname:port:password of the redis server that data for this table should come from. (e.g. athena-federation-demo.cache.amazonaws.com:6379) Alternatively, you can store the endpoint or part of the endpoint in SecretsManager by using ${secret_name} as the table property value. IMPORTANT: If you have an encrypted redis cluster, you must add the prefix 'rediss://' to your hostname. (eg. rediss://athena-federation-demo.cache.amazonaws.com:6379:password)
