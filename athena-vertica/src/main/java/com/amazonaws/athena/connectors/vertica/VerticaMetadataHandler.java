...
import com.amazonaws.athena.connectors.vertica.query.VerticaSqlDialect;
...
/**
 * Handles Athena federation metadata operations for the Vertica connector: schema, table, and split listing and,
 * with Substrait, query-plan-driven EXPORT.
 */
        ...
        extends JdbcMetadataHandler
{
            ...
            throws Exception
    {
        ...
        try (Connection client = getJdbcConnectionFactory().getConnection(getCredentialProvider(getRequestOverrideConfig(request)))) {
            ...
            try (ResultSet rs = dbMetadata.getTables(null, null, null, TABLE_TYPES)) {
                while (rs.next())
                {
    ...
    public void getPartitions(BlockWriter blockWriter, GetTableLayoutRequest request, QueryStatusChecker queryStatusChecker) throws Exception {
        ...
        try (Connection connection = getJdbcConnectionFactory().getConnection(getCredentialProvider(getRequestOverrideConfig(request)))) {
            ...
            if (!request.getTableName().getQualifiedTableName().equalsIgnoreCase(queryPassthrough.getFunctionSignature())) {
                ...
                try (ResultSet definition = dbMetadata.getColumns(null, tableName.getSchemaName(), tableName.getTableName(), null)) {

                    // Use Substrait query plan if available - this path converts the pre-compiled
                    // query plan from Athena into Vertica-compatible SQL using Calcite's SqlDialect system
    ...
    public GetSplitsResponse doGetSplits(BlockAllocator allocator, GetSplitsRequest request){
        ...
        try (Connection connection = getJdbcConnectionFactory().getConnection(getCredentialProvider(getRequestOverrideConfig(request)))) {
            ...
            throw new RuntimeException("doGetSplits failed ", e);
        }
    }
    ...
    @VisibleForTesting
    void testAccess(Connection conn, TableName table) {
        ST simpleTestSqlST = new ST("SELECT * FROM \"<schemaName>\".\"<tableName>\" LIMIT 1;");
        simpleTestSqlST.add("schemaName", escapeIdentifier(table.getSchemaName()));
        simpleTestSqlST.add("tableName", escapeIdentifier(table.getTableName()));
        ...
        try (PreparedStatement testAccessSql = conn.prepareStatement(simpleTestSqlST.render());
            ...
            // Statement and ResultSet are auto-closed
        } catch (Exception e) {
    ...
    /** Doubles embedded double quotes in an identifier to prevent SQL injection in {@link #testAccess}. */
