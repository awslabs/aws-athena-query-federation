...
public class Db2As400MetadataHandler extends JdbcMetadataHandler
{
     ...
     */
    public Db2As400MetadataHandler(java.util.Map<String, String> configOptions)
    {
     ...
     */
    public Db2As400MetadataHandler(DatabaseConnectionConfig databaseConnectionConfig, java.util.Map<String, String> configOptions)
    {
        this(databaseConnectionConfig, new GenericJdbcConnectionFactory(databaseConnectionConfig, null, new DatabaseConnectionInfo(Db2As400Constants.DRIVER_CLASS, Db2As400Constants.DEFAULT_PORT)), configOptions);
    }
     ...
     */
    public Db2As400MetadataHandler(
    ...
    public ListSchemasResponse doListSchemaNames(final BlockAllocator blockAllocator, final ListSchemasRequest listSchemasRequest) throws Exception
    {
        try (Connection connection = getJdbcConnectionFactory().getConnection(getCredentialProvider())) {
            ...
            return ListSchemasResponse.newBuilder().setCatalogName(listSchemasRequest.getCatalogName()).addAllSchemas(getSchemaList(connection, Db2As400Constants.QRY_TO_LIST_SCHEMAS)).build();
        }
    }
        ...
        try (Connection connection = getJdbcConnectionFactory().getConnection(getCredentialProvider());
            ...
            if (!resultSet.next()) {
                ...
                do {
                    ...
                    blockWriter.writeRows((Block block, int rowNum) ->
                    {
                        block.setValue(PARTITION_NUMBER, rowNum, columnName + ":::" + resultSet.getString("TABLE_PARTITION"));
                        //we wrote 1 row so we return 1
                        return 1;
                    });
