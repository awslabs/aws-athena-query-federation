 ...
 * #L%
 */
package com.amazonaws.athena.connectors.kafka.integ;
...
public class KafkaIntegTest extends IntegrationTestBase {
    private static final Logger logger = LoggerFactory.getLogger(KafkaIntegTest.class);
    private final App theApp;
    private final String lambdaFunctionName;

    private static TestConfig testConfig;
    private static  String kafka_db_name;
    private static  String tableName;
    private String lambdaAndkafka_db_name;

    private static  String region;
    static {
        testConfig = new TestConfig();
        region = testConfig.getStringItem("region").get();
        kafka_db_name = testConfig.getStringItem("kafka_db_name").get();
    ...
    public KafkaIntegTest()
    {
        logger.warn("Entered constructor");
        theApp = new App();
        lambdaFunctionName = getLambdaFunctionName();
        lambdaAndkafka_db_name = "\"lambda:"+lambdaFunctionName+"\""+kafka_db_name+"\"";
    }
    ...
    public void fetchTableData()
    {
        ...
        logger.info("-----------------------------------");
        List tableNames = fetchDataSelect(kafka_db_name,tableName,lambdaFunctionName);
    ...
    public void fetchTableCount()
    {
        ...
        logger.info("-----------------------------------");
        List tableNames = fetchDataSelectCountAll(kafka_db_name,tableName,lambdaFunctionName);
        logger.info("Total Records: {}", tableNames.size());
    }
    @Test
    ...
    public void fetchDataWhereClause()
    {
        ...
        logger.info("-----------------------------------");
        List tableNames = fetchDataWhereClause(kafka_db_name,tableName,lambdaFunctionName,"name","varun");
    ...
    public void fetchDataWhereClauseLIKE()
    {
        ...
        logger.info("-----------------------------------");
        List tableNames = fetchDataWhereClauseLIKE(kafka_db_name,tableName,lambdaFunctionName,"name","namrata");
