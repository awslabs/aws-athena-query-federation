...
public class ImpalaMuxRecordHandlerTest
{
    ...
    public void setup()
    {
        ...
        this.jdbcRecordHandler = new ImpalaMuxRecordHandler(this.amazonS3, this.secretsManager, this.athena, this.jdbcConnectionFactory, databaseConnectionConfig, this.recordHandlerMap, com.google.common.collect.ImmutableMap.of("spill_bucket", "asdf_spill_bucket_loc"));
    ...
    public void readWithConstraint() throws Exception
    {
        ...
        ReadRecordsRequest readRecordsRequest = ReadRecordsRequest.newBuilder().setCatalogName("recordImpala").build();
        this.jdbcRecordHandler.readWithConstraint(blockAllocator, blockSpiller, readRecordsRequest, queryStatusChecker);
        Mockito.verify(this.impalaRecordHandler, Mockito.times(1)).readWithConstraint(Mockito.eq(blockAllocator), Mockito.eq(blockSpiller), Mockito.eq(readRecordsRequest), Mockito.eq(queryStatusChecker));
    }
