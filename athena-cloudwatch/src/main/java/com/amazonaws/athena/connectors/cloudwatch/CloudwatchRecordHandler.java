            ...
            for (List<ResultField> resultList : getQueryResultsResult.getResults()) {
                spiller.writeRows((Block block, int rowNum) -> {
                    ...
                    for (ResultField resultField : resultList) {
                        if (resultField.getField().equalsIgnoreCase("@timestamp")) {
                            logger.info("field {} , value {}", resultField.getField(), resultField.getValue());
                                matched &= block.offerValue(LOG_TIME_FIELD, rowNum, new SimpleDateFormat("yyyy-MM-dd HH:mm:ss.SSS").parse(resultField.getValue()).getTime());
                            }
                        else if (resultField.getField().equalsIgnoreCase("@message")) {
                            logger.info("field {} , value {}", resultField.getField(), resultField.getValue());
                            matched &= block.offerValue(LOG_MSG_FIELD, rowNum, resultField.getValue());
                        }
                    }
                    return matched ? 1 : 0;
                });
