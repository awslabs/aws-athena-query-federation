WITH year_total AS (
  SELECT
    ...
    's' sale_type
  FROM customer[ TABLE_SUFFIX ], store_sales[ TABLE_SUFFIX ], date_dim[ TABLE_SUFFIX ]
  ...
  SELECT
    ...
    'c' sale_type
  FROM customer[ TABLE_SUFFIX ], catalog_sales[ TABLE_SUFFIX ], date_dim[ TABLE_SUFFIX ]
  ...
  SELECT
    ...
    sum((((ws_ext_list_price - ws_ext_wholesale_cost - ws_ext_discount_amt) + ws_ext_sales_price) /
      2)) year_total,
    'w' sale_type
  FROM customer[ TABLE_SUFFIX ], web_sales[ TABLE_SUFFIX ], date_dim[ TABLE_SUFFIX ]
