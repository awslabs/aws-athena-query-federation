        ...
        extends GlueMetadataHandler
{
     ...
     * just return the glue database name provided as a single database (schema) name.
    ...
    public ListSchemasResponse doListSchemaNames(BlockAllocator allocator, ListSchemasRequest request)
    {
        ...
        // TODO: The below code can be cleaned up if not required in the long run. 
        // This code fetches neptune cluster endpoint name from env variable and frames schema (database) name based 
        // on that.
        // String neptuneEndpoint = System.getenv("neptune_endpoint");
        // String[] splitEndpointName = neptuneEndpoint.split("\\.");
        // schemas.add(splitEndpointName[0]);
     ...
     * In this case, fetch list of tables in the Glue database provided.
     * 
     * @param allocator Tool for creating and managing Apache Arrow Blocks.
    ...
    public ListTablesResponse doListTables(BlockAllocator allocator, ListTablesRequest request)
    {
        ...
        /**
         * TODO: Clean up the commented code below when no longer required
         *
         */
        //tables.add(new TableName(request.getSchemaName(), "table1"));
        //Get list of unique Vertex label names
        // GraphTraversal<Vertex, String> result = g.V().label().dedup();
        //     result.forEachRemaining(
        //         e ->  tables.add(new TableName(request.getSchemaName(), e))
        //     );
        ...
        glueTableList.forEach(
            e -> tables.add(new TableName(request.getSchemaName(), e.getName()))
        );
    ...
    public GetTableResponse doGetTable(BlockAllocator blockAllocator, GetTableRequest request) throws Exception
    {
        ...
        //TODO: ***************Cleanup below code after it's deemed to be unnecessary **********************
     //   Set<String> partitionColNames = new HashSet<>();
        ...
        /**
         * Reference dataset fields from air-routes-latest-nodes.csv
         * ~id,~label,type:string,code:string,icao:string,
         * desc:string,region:string,runways:int,longest:int,elev:int,country:string,
         * city:string,lat:double,lon:double,author:string,date:string
         */
