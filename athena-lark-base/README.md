...
| `default_does_activate_complex_type_as_json_string` | No | Represent List/Struct-shaped columns (Multi Select, User, Attachment, Url, Location, Single/Duplex Link, Group Chat, Lookup, Created/Modified User) as a JSON string instead, sidestepping the `WHERE`-on-List/Struct crash described under [Known Limitations](#known-limitations). Only consulted for the live/experimental (non-Glue) schema paths; for a Glue-backed table, set it on the crawler instead (default: false) |
...
- **`Number` field precision**: Lark Base stores `Number` values as IEEE 754 doubles (about 15-17 significant digits). Longer values (bank card numbers, long numeric IDs) are rounded by Lark itself before they reach the connector. Store such values in a `Text` field.
- **`WHERE` on List/Struct-typed columns fails the query**: any `WHERE` clause (even `IS NOT NULL`) on an array/struct-shaped column (`Multi Select`, `User`, `Attachment`, `Url`, `Location`, `Single Link`, `Duplex Link`, `Group Chat`, `Lookup`, `Created User`, `Modified User`, or a `Formula`/`Lookup` resolving to one) fails with `GENERIC_INTERNAL_ERROR: ... Lists have one child Field. Found: none`. This happens inside Athena's query engine before the connector is invoked for splits or records, so the connector cannot intercept it. Either filter on a scalar column instead, or set `default_does_activate_complex_type_as_json_string=true` to expose these columns as JSON strings (queryable with `json_extract`/`json_extract_scalar`). The setting is off by default. For Glue-crawled tables, the Glue column type is decided by the crawler, so set the variable on the Glue Lark Base Crawler.
