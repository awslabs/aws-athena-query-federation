...
public class SynapseMuxRecordHandlerTest
{
    ...
    public void setup()
    {
        this.synapseRecordHandler = Mockito.mock(SynapseRecordHandler.class);
        this.recordHandlerMap = Collections.singletonMap(SynapseConstants.NAME, this.synapseRecordHandler);
        ...
        this.jdbcRecordHandler = new SynapseMuxRecordHandler(this.amazonS3, this.secretsManager, this.athena, this.jdbcConnectionFactory, databaseConnectionConfig, this.recordHandlerMap, com.google.common.collect.ImmutableMap.of("spill_bucket", "asdf_spill_bucket_loc"));
            ...
            throws Exception
    {
        ...
        ReadRecordsRequest readRecordsRequest = ReadRecordsRequest.newBuilder().setCatalogName(SynapseConstants.NAME).build();
        this.jdbcRecordHandler.readWithConstraint(blockAllocator, blockSpiller, readRecordsRequest, queryStatusChecker);
        Mockito.verify(this.synapseRecordHandler, Mockito.times(1)).readWithConstraint(Mockito.eq(blockAllocator), Mockito.eq(blockSpiller), Mockito.eq(readRecordsRequest), Mockito.eq(queryStatusChecker));
    }
