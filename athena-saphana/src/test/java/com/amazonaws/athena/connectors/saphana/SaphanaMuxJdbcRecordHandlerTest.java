...
public class SaphanaMuxJdbcRecordHandlerTest
{
            ...
            throws Exception
    {
        ...
        ReadRecordsRequest readRecordsRequest = ReadRecordsRequest.newBuilder().setCatalogName("saphana").build();
        this.jdbcRecordHandler.readWithConstraint(blockAllocator, blockSpiller, readRecordsRequest, queryStatusChecker);
        Mockito.verify(this.saphanaRecordHandler, Mockito.times(1)).readWithConstraint(Mockito.eq(blockAllocator), Mockito.eq(blockSpiller), Mockito.eq(readRecordsRequest), Mockito.eq(queryStatusChecker));
    }
