...
public class SaphanaCredentialsProvider implements CredentialsProvider
{
    ...
    public Map<String, String> getCredentialMap()
    {
        try {
            ...
            throw new RuntimeException("Error retrieving SAP HANA credentials: " + ex.getMessage(), ex);
        }
    }
    ...
    {
        String accessToken;
        String clientId = Validate.notNull(oauthConfig.get(SaphanaConstants.CLIENT_ID), "Missing required property: client_id");
        String tokenEndpoint = Validate.notNull(oauthConfig.get(SaphanaConstants.TOKEN_URL), "Missing required property: token_url");
        String clientSecret = Validate.notNull(oauthConfig.get(SaphanaConstants.CLIENT_SECRET), "Missing required property: client_secret");

        accessToken = loadTokenFromSecretsManager(oauthConfig);

        if (accessToken == null) {
            LOGGER.debug("First time auth. Using client_credentials grant...");
            JSONObject tokenJson = getTokenFromClientCredentials(tokenEndpoint, clientId, clientSecret);
            ...
            if ((now - fetchedAt) < expiresIn - 60) {
                ...
                JSONObject refreshed = getTokenFromClientCredentials(tokenEndpoint, clientId, clientSecret);
    ...
    private JSONObject getTokenFromClientCredentials(String tokenEndpoint, String clientId, String clientSecret) throws Exception 
    {
        ...
        if (responseCode != 200) {
            throw new RuntimeException("Failed to obtain access token: " + responseCode + " - " + responseBody);
        }
