        ...
        extends DefaultJDBCCaseResolver
{
    ...
    /*
    *  Saphana previous behavior was defaulting to upper case if no annotation found under ANNOTATION mode.
    *  Keeping this default behavior...
     */
    @Override
    protected TableName getTableNameFromQueryAnnotation(TableName table)
    {
        LOGGER.debug("Saphana: getTableNameFromQueryAnnotation: " + table);
        // Saphana previous behavior was defaulting to upper case if no annotation found under ANNOTATION mode.
        // Keeping this default behavior...
        if (!table.getTableName().contains("@")) {
            LOGGER.info("Saphana: ANNOTATION enabled, but no ANNOTATION found, back to original behavior UPPER_CASE.: " + table);
