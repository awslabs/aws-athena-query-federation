...
public enum DataLakeGen2SQLDataType {
    ...
    private static final Map<String, DataLakeGen2SQLDataType> DATALAKE_GEN2_SQL_DATA_TYPE_MAP = new HashMap<>();

    static {
        for (DataLakeGen2SQLDataType next : values()) {
            DATALAKE_GEN2_SQL_DATA_TYPE_MAP.put(next.gen2Type, next);
        }
    }
    ...
    DataLakeGen2SQLDataType(String gen2Type, ArrowType arrowType)
    {
    ...
    {
        DataLakeGen2SQLDataType result = DATALAKE_GEN2_SQL_DATA_TYPE_MAP.get(gen2Type);
        return result.arrowType;
    }

    public static boolean isSupported(String datatype)
    {
        for (DataLakeGen2SQLDataType sqlServerSQLDataType : values()) {
