...
public class GcsRecordHandlerTest {
    ...
    private GcsRecordHandler gcsRecordHandler;
    ...
    public void testReadWithConstraint()
        ...
        Split split = new Split(S3SpillLocation.newBuilder()
                .withBucket(bucket)
                .withPrefix(prefix)
                ...
                .build(),
                keyFactory.create(), Map.of("storage_split_json", "{\"fileName\": \"test\"}"));
...
//                return l;
//            }).when(csvMetadata).readRecords(any(), any(), any(),
//                    any(), any(), any());
            FileSystemDatasetFactory fsdsFactory = mock(FileSystemDatasetFactory.class);
            PowerMockito.whenNew(FileSystemDatasetFactory.class).withAnyArguments().thenReturn(fsdsFactory);
