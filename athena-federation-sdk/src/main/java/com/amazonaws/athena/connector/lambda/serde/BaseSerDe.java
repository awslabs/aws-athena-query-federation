...
public abstract class BaseSerDe<T>
{
    private static final String TYPE_FIELD = "@type";

    /**
     * Serializes an object to an enclosed JSON object. Delegates actual serialization logic to {@link #doSerialize(JsonGenerator, Object)} and wraps it in braces.
     *
    ...
    /**
     * Deserializes an enclosed JSON object to a Java object. Unwraps the enclosed JSON object and delegates actual deserialization logic to {@link #doDeserialize(JsonParser)}.
     *
    ...
    /**
     * Serializes an object to JSON. Implementers of this method should not enclose/wrap their objects with braces.
     *
     * @param jgen The JSON generator.
     * @param object The object to be serialized.
     * @throws IOException If there is an error generating the JSON.
     */
    public abstract void doSerialize(JsonGenerator jgen, T object)
            throws IOException;

    /**
     * Deserializes an object from JSON. Implementers of this method should not unwrap or parse enclosing braces.
     *
     * @param jparser The parser to use for extraction.
     * @return The deserialized Java object.
     * @throws IOException If there is an error parsing the field.
     */
    public abstract T doDeserialize(JsonParser jparser)
            throws IOException;

    /**
     * Helper used to write the type of the object (usually the class name). Here for convenience and consistency with {@link #getType(JsonParser)}.
     *
     * @param jgen The JSON generator.
     * @param type The type (class) name to serialize.
     * @throws IOException If there is an error generating the JSON.
     */
    protected void writeType(JsonGenerator jgen, String type)
            throws IOException
    {
        jgen.writeStringField(TYPE_FIELD, type);
    }
