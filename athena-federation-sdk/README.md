...
Each connector exposes to Athena which capabilities it is able to push down to the underlying datasource in the MetadataHandler's doGetDataSourceCapabilities API. These capabilities include simple filters (such as colA > 10), complex expressions (such as colA + colB < colC), aggregation expressions (such as select MAX(col) ... GROUP BY col), order by clauses (such as ORDER BY colB ASC, colC DESC), and limit fields (such as LIMIT 100). Athena expects each connector to correctly push down each of these constraints to the underlying data source in the RecordHandler's readWithConstraint API.
