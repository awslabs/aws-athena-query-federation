...
2. From the athena-slack-member-analytics dir, run `deploy_dummy_endpoint.sh`. This step will deploy a REST endpoint with API Gateway and Lambda using your environment AWS profile. It will also create an AWS Secret with a dummy token and initialize the following environment variables:

```
    export data_endpoint="<API_Gateway_Endpoint>" 
    export region="<AWS_Region>"
    export secret_name="<New_AWS_Secret_Name>"
    export auth_token="<Dummy_Token>"
