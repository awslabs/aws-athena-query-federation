...
class MySQL_Federated_Source(JDBC_Source, connection_type_name="mysql", is_rds_source=True):
    def get_glue_job_name(self):
        return "athena_integ_s3_to_mysql"

    def get_glue_script_name(self):
        return "s3_to_mysql.py"

    def get_glue_etl_connection_name(self):
        return "athena_integ_s3_to_mysql_glue_connection"
    ...
    def create_jdbc_engine(self, with_db: bool=True):
        ...
        if with_db:
            return create_engine(f"mysql+pymysql://{credential['username']}:{credential['password']}"
                               f"@{rds_client.get_rds_db_host(config_helper.get_rds_database_identifier_name(self.get_connection_type_name()))}:3306/{config_helper.get_tpcds_scale_factor()}")
        else:
            return create_engine(f"mysql+pymysql://{credential['username']}:{credential['password']}"
                               f"@{rds_client.get_rds_db_host(config_helper.get_rds_database_identifier_name(self.get_connection_type_name()))}:3306")
    ...
    def start_glue_job(self, table_names: list):
        ...
        job_arguments = {
            ...
            '--MYSQL_URL': f'jdbc:mysql://{host_name}:3306/{config_helper.get_tpcds_scale_factor()}',
            '--MYSQL_USER': credential['username'],
            '--MYSQL_PWD': credential['password']
        }
            ...
            "connection-properties": json.dumps(
                {
                    ...
                    "SubnetId": self.get_subnet_1_id(),
                    "database": config_helper.get_tpcds_scale_factor(),
                    "port": 3306,
